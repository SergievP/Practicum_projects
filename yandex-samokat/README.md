...
**Цель тестирования:**
Проверка корректности работы web-версии, мобильного приложения и API сервиса аренды самокатов, выявление функциональных и логических дефектов.
...
- Логика отправки и отображения push-уведомлений
- Обработка переходов из уведомлений
- Поведение приложения при отсутствии интернет-соединения
- Соответствие верстки макетам
...
- Google Chrome, Яндекс.Браузер - кроссбраузерное тестирование
...
- Выполнено позитивное, негативное тестирование
...
### Баг №1 — Несоответствие даты доставки в форме заказа и статусе заказа
...
![Несоответствие даты доставки в форме заказа и статусе заказа](Screenshots/bug-1.png)

### Баг №2 — Отправка push-уведомления после завершения заказа
...
### Баг №3 — Заказы курьера не удаляются после удаления учетной записи
...
![Заказы курьера не удаляются после удаления учетной записи](Screenshots/bug-3.png)

## Вывод 
В ходе тестирования были выявлены дефекты различного уровня критичности — от UI-недочетов до блокирующих и критических ошибок, влияющих на оформление заказа, корректность данных и работу уведомлений.

Результаты показывают, что продукт требует доработки в части:
- валидации пользовательского ввода,
- синхронизации бизнес-логики между клиентом и сервером,
- корректной обработки статусов и системных событий.

Проведенное тестирование позволило выявить ключевые зоны риска и обеспечить прозрачность текущего состояния качества системы.